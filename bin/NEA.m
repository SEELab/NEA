...
% Brian D. Fath & Stuart R. Borrett 2004
 % 
 % SRB || MODIFIED: March 3, 2007 to work with Octave
 % =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
% I. INTRODUCTION
% =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=

% This program is a compilation of algorithms for network environ analysis.
% This is a self-contained program; all functions required are included in
% this file.
%
% REQUIRED DATA INPUT 

% The input variable 'data' is an (n+1 x n+2) matrix composed of an nxn flow
% matrix (F), an nx1 input vector (z), an nx1 storage vector (x), a 1xn
% output vector (y), and a 1x2 vector of zeros.  Data should have consistant
% units and reflect a system at steady-state (though some analyses remain
% valid for non-steadystate (i.e., structural analysis)).  .
%
% DATA OUTPUT

% 'ep' is a vector of environ properties and network statistics.  To return
% additional variables to the Matlab workspace, place the variable name into
% the output definition.  For example, [A,G,ep]=NEA(data) will return A, G
% and ep to the workspace. All resultant variables are stored in
% NEA_output.mat.
%
% PROGRAM OUTLINE
% I.	Introduction
...
if length(varargin)==0, disp_ctrl=1; % shows all
elseif length(varargin)==1, disp_ctrl=varargin{1}; 
end

global n I
[F, y, z, x]=unpack(data);  % Unpacks the data matrix into component parts
n=length(F);		    % length of F gives the dimensions of the
                            % flow matrix
T=sum(F')'+z;		    % total throughflow at each compartment
                            % including input
...
ss_tol=5e-2;        % steady-state tolerance facor
...
HFP=[CVGP/CVNP];			        	% homogenization parameter (input)
